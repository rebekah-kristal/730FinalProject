...
```
# Model fitting
Note: using brm for hierarchical models, and will only compare these using our PPC plots; stan_glm didn't like the "|" specification, and haven't yet figured out a workaround
...
```{r}
modSLoo <- stan_glm(y~REGION + AgeCat + SEX + RACENEW + EduCat + POORYN,
  data = newdata1, 
  weights = Weight,
...
```{r}
modRLoo2 <- stan_glm(
  y~REGION + AgeCat + SEX + RACENEW + EduCat + POORYN + REGION*POORYN + REGION*RACENEW + RACENEW*POORYN, 
  data = newdata1,
  weights = Weight,
  family=cumulative(link="logit"),  
  iter = 1000, 
